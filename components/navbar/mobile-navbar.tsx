          ...
          >
            ...
            <div className="flex flex-col gap-8 px-8 py-8 flex-1 text-left">
              {navItems.map((item: any, idx: number) => {
                ...
                if (item.highlight) {
                  const Icon = IconMap[item.icon] || Baby;
                  return (
                    <div key={idx} className="w-full text-left">
                      <Link
                        href={item.link}
                        onClick={() => setOpen(false)}
                        className="flex items-center gap-3 p-4 bg-gradient-to-r from-amber-400 to-orange-500 text-white rounded-2xl shadow-lg shadow-orange-500/20 active:scale-95 transition-transform"
                      >
                        <Icon size={24} />
